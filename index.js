...
app.get("/users/:id", (req, res) => {
    const data = readData(); // Función para leer users
    const id = parseInt(req.params.id); // Para extraer el id del objeto de la petición
...
app.post("/users", (req, res) => {
    ...
    if (Object.keys(body).length  < 1){
        return res.status(400).json({message: "No se encuentran datos a registrar."})
    }
...
app.put("/users/:id", (req, res)=>{
    ...
    if (userIndex  === -1) { //Cuando no lo encuentra en -1
        res.status(204).json({message: `El Usuario con Id ${id} no existe` });
    }
